...
def load_model():
    ...
    # ── KEY FIX: use tf.keras not standalone keras ──
    ...
    # ── KEY FIX: load with safe_mode=False and custom_objects ──
...
if uploaded is not None:
    ...
    display_cols = [c for c in ["order_id", "payment_value", "expected_total",
    ...
    if st.button("Detect Leakage", type="primary"):
        ...
        c2.metric("Flagged as leakage", f"{n_flagged}",
                  delta=f"{rate:.2f}% of orders", delta_color="inverse")
        ...
        colors_map = {"Critical": "#E24B4A", "High": "#EF9F27",
                      "Medium": "#378ADD", "Low": "#1D9E75"}
        clrs = [colors_map.get(r, "#888780") for r in risk_counts["Risk Level"]]
        ...
        ax2.hist(normal_scores, bins=60, color="#378ADD", alpha=0.6,
                 label="Normal", density=True)
        if len(flagged_scores) > 0:
            ax2.hist(flagged_scores, bins=20, color="#E24B4A", alpha=0.8,
                     label="Flagged", density=True)
        ax2.axvline(config["vae_threshold"], color="#EF9F27", linewidth=2,
                    linestyle="--",
        ...
        if len(flagged_df) == 0:
            ...
            st.dataframe(
                flagged_df.style.applymap(colour_risk, subset=["risk_level"]),
        ...
        if "actual_label" in results.columns:
            ...
            report = classification_report(
                results["actual_label"], results["leakage_flag"],
                target_names=["Normal", "Leakage"], output_dict=True)
            st.dataframe(pd.DataFrame(report).T.round(3))

            fig3, ax3 = plt.subplots(figsize=(5, 4))
            cm = confusion_matrix(results["actual_label"], results["leakage_flag"])
    ...
    st.info("No file uploaded yet. Upload your orders CSV above to get started.")
    st.markdown("""
