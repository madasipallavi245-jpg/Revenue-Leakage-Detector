...
================================================================
  ...
  Fixed: VAE class rebuilt to exactly match leakage_vae_fixed.keras
================================================================
"""
...
def engineer_features(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Target (derived but not used as input)
    ...
    # KEY FIX 2 features — the primary leakage signal
...
if uploaded is not None:
    ...
    if st.button("Detect Leakage", type="primary"):
        ...
        clrs = [colors_map.get(str(r), "#888780") for r in risk_counts["Risk Level"]]
        ax.barh(risk_counts["Risk Level"].astype(str),
                risk_counts["Count"], color=clrs, edgecolor="none")
        ...
        if len(flagged_df) == 0:
            ...
            st.dataframe(
                flagged_df.style.map(colour_risk, subset=["risk_level"]),
                use_container_width=True,
            )

        # Actual vs predicted (if labels present)
